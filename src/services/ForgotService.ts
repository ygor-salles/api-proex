...
class ForgotService {
    public async execute(email: string) {
        ...
        if (updatePassword) {
            ...
            if (sendEmail) {
                return { status: httpStatus.OK, message: 'Senha atualizada com sucesso, verifique sua caixa de entrada' };
            }
            return { status: httpStatus.BAD_REQUEST, message: 'Senha atualizada, mas falha no envio de e-mail' }
        } else {
            return { status: httpStatus.BAD_REQUEST, message: 'Erro ao resetar senha' }
        }
    }

    private async sendMailForgotPassword(email: string, name: string, password: string): Promise<any> {
